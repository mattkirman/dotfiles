...
parse_git_dirty() {
  ...
  if [[ -n $(git status -s ${SUBMODULE_SYNTAX} -uno  2> /dev/null) ]]; then
    echo "$ZSH_THEME_GIT_PROMPT_PREFIX_DIRTY${ref#refs/heads/}$ZSH_THEME_GIT_PROMPT_DIRTY"
  else
    echo "$ZSH_THEME_GIT_PROMPT_PREFIX_CLEAN${ref#refs/heads/}$ZSH_THEME_GIT_PROMPT_CLEAN"
  fi
}
